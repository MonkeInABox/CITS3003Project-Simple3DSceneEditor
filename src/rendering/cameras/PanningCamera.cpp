...
PanningCamera::PanningCamera()
    : distance(init_distance), focus_point(init_focus_point), pitch(init_pitch),
      yaw(init_yaw), near(init_near), fov(init_fov) {}

PanningCamera::PanningCamera(float distance, glm::vec3 focus_point, float pitch,
                             float yaw, float near, float fov)
    : init_distance(distance), init_focus_point(focus_point), init_pitch(pitch),
      init_yaw(yaw), init_near(near), init_fov(fov), distance(distance),
      focus_point(focus_point), pitch(pitch), yaw(yaw), near(near), fov(fov) {}

void PanningCamera::update(const Window &window, float dt,
                           bool controls_enabled) {
  if (controls_enabled) {
    bool ctrl_is_pressed = window.is_key_pressed(GLFW_KEY_LEFT_CONTROL) ||
                           window.is_key_pressed(GLFW_KEY_RIGHT_CONTROL);
    ...
    if (!resetSeq) {
      // Extract basis vectors from inverse view matrix to find world space
      // directions of view space basis
      ...
      focus_point += (x_basis * (float)-pan.x + y_basis * (float)pan.y) *
  ...
  //   view_matrix =
  //       glm::translate(glm::vec3(glm::vec4{0.0f, 0.0f, -distance, 0.f} *
  //                                glm::rotate(yaw, glm::vec3{0, 0, 1}) *
  ...
  projection_matrix = glm::infinitePerspective(
      fov, window.get_framebuffer_aspect_ratio(), near);
...
void PanningCamera::add_imgui_options_section(
  ...
  ImGui::SliderFloat("Near Plane", &near, 0.001f, 1.0f, "%.3f",
...
CameraProperties PanningCamera::save_properties() const {
  return CameraProperties{get_position(), yaw, pitch, fov, gamma};
}
...
void PanningCamera::load_properties(const CameraProperties &camera_properties) {
  ...
  auto inverse_rot_matrix = glm::rotate(yaw, glm::vec3{0.0f, 1.0f, 0.0f}) *
...
glm::mat4 PanningCamera::get_view_matrix() const { return view_matrix; }
...
float PanningCamera::get_gamma() const { return gamma; }
