...
//       point_lights_ubo({}, false), directional_lights_ubo({}, false) { get_uniforms_set_bindings(); }
...
BaseHeightmapEntityShader::BaseHeightmapEntityShader(std::string name, const std::string &vertex_path, const std::string &fragment_path,
                                                     std::unordered_map<std::string, std::string> vert_defines,
                                                     std::unordered_map<std::string, std::string> frag_defines)
    : BaseLitEntityShader(std::move(name), vertex_path, fragment_path, std::move(vert_defines), std::move(frag_defines)) {}

HeightmapEntityRenderData::HeightmapEntityRenderData(std::shared_ptr<TextureHandle> diffuse_texture, std::shared_ptr<TextureHandle> specular_map_texture)
    : BaseLitEntityRenderData(std::move(diffuse_texture), std::move(specular_map_texture)) {
    heightmap_texture = create_noise_texture(16, 1.f, 0);
}

void BaseHeightmapEntityShader::get_uniforms_set_bindings() {
    BaseLitEntityShader::get_uniforms_set_bindings();
    set_binding("heightmap_texture", 2);
}
...
//         directional_lights_ubo.data[i].facing_direction = glm::vec4(1.f, 0.f, 0.f, 0.f) * glm::toMat4(directional_light.rotation);
//     }
