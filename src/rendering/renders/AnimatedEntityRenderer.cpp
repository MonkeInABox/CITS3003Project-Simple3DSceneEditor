...
AnimatedEntityRenderer::AnimatedEntityShader::AnimatedEntityShader() : BaseLitEntityShader("Animated Entity", "animated_entity/vert.glsl", "animated_entity/frag.glsl", {{"BONE_TRANSFORMS", BONE_TRANSFORMS_STR}}) {
    ...
    for (const auto &entity : render_scene.entities) {
        ...
        entity->mesh_hierarchy->visit_nodes([this, &entity](const MeshHierarchyNode &node, glm::mat4 accumulated_transformation) {
            for (const auto &mesh_id : node.meshes) {
                ...
                if (!mesh.bone_transforms.empty())
                    shader.set_bone_transforms(mesh.bone_transforms);
