...
class BasicStaticScene : public SceneInterface {
  ...
  /// The initial camera settings, which is where the camera will be reset to
  /// when pressing (R)
  ...
  enum class CameraMode { Panning, Flying } camera_mode = CameraMode::Panning;
...
public:
  ...
  std::pair<TickResponseType, std::shared_ptr<SceneInterface>>
  tick(float delta_time, const SceneContext &scene_context) override;
